...
opt.data_path = '/nfs/data3/gergo/Neurofinder_update/neurofinder.02.00/images/image00001.tiff';
...
% % W initialisation
opt.init_model = 'pointlike'; %'donut_conv'; %'filled'; %{'filled', 'donut'};
% load('/nfs/data3/gergo/Neurofinder_update/neurofinder.02.00/CHOMP/output/neurofinder.02.00_20170815T072504_iter_7.mat',...
%   'model');
% cur_W = model.W;
% cur_W = reshape(cur_W, sqrt(size(cur_W,1)), sqrt(size(cur_W,1)), []);
% final_W = zeros(opt.m^2, opt.KS);
% for i1 = 1:min(size(cur_W,3), size(final_W,2)), final_W(:,i1) = reshape(imresize(cur_W(:,:,i1),[opt.m, opt.m]),1,[]); end
% opt.init_W = final_W;
...
opt.mask_image(:,1:round(30*opt.spatial_scale)) = 0;
opt.mask_image(:,(end-round(30*opt.spatial_scale)):end) = 0;
...
% opt.data_path = '/nfs/data3/gergo/Neurofinder_update/neurofinder.02.01/images/image00001.tiff';
% % Set the subfolders names - neurofinder specific
% opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
% opt.output_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/output/'];
% opt.results_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/results/'];
...
% Set the new data paths
opt.data_path = '/nfs/data3/gergo/Neurofinder_update/neurofinder.02.00.test/images/image00001.tiff';
% Set the subfolders names - neurofinder specific
opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
opt.output_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/output/'];
opt.results_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/results/'];
...
% Set the new data paths
opt.data_path = '/nfs/data3/gergo/Neurofinder_update/neurofinder.02.01.test/images/image00001.tiff';
% Set the subfolders names - neurofinder specific
opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
opt.output_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/output/'];
opt.results_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/results/'];
