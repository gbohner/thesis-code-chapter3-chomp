...
chomp_timestamp = '20190529T010717'; chomp_iter = '2'; % after recent fixes %iter2 diag_cumulants=0, iter3 diag_cumulants=1 % iter 4 with extra_diag
%chomp_timestamp = '20190529T010021'; chomp_iter = '4';%small test run
%chomp_iter = '1';
use_cells = 1:400;
...
use_cells = [72,73,74,76]; % iter 2 
%use_cells = 48:51; % iter 3
%use_cells = [57:61]; % iter 4
%use_cells = [7,12];%12,56,80]; % iter 5
%use_cells = [27,40,51]; %smallexample
...
cur_area = [268,   427,   308,   453]; % iter 2
%cur_area = [272,   426,   303,   452]; % iter 3
...
if ~env_home
  ...
  save(['./Examples/reconst_example_' dataset_name '_' chomp_timestamp '.mat'], ...
    'full_reconst_diag', 'full_reconst_covmat', ...
    'data_mean', 'data_cov', 'szPatch');
  end
else % if on laptop, just load the saved data
  if ~exist(['~/Data/CHOMP_Examples/reconst_example_' dataset_name '_' chomp_timestamp '.mat'], 'file')
    copyfile(['./Examples/reconst_example_'  dataset_name '_' chomp_timestamp '.mat'], ...
      ['~/Data/CHOMP_Examples/reconst_example_'  dataset_name '_' chomp_timestamp '.mat']);
  end
  load(['~/Data/CHOMP_Examples/reconst_example_'  dataset_name '_' chomp_timestamp '.mat'])
end
...
%    mom_mode = get_hist_mode(opt.cumulant_pixelwise{mom1},500);
%   clim = [0,6e6];
