function [out_timestamp, saved_opts] = func_dataset_03_preproc2P(preproc2P_stamp, input_opts)
...
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.03.00' ...
...
opt.KS = 2;
opt.mom = 1;
%opt.mom_weights = [0,1,.3,1]; %[0,1,1]; %[0, 0, 0, 1];
opt.init_model = 'neurofinder_preproc2P'; %'donut_conv'; %'neurofinder_preproc2P'; %{'donut_four'}; %{'donut_conv', 'pointlike'};%{'donut_four', 'pointlike'}; %'donut_two'; %'donut_conv'; %'pointlike'; %'filled'; %{'filled', 'donut'};
...
% Set the new data paths
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.03.00.test' ...
  '/preproc2P/images' preproc2P_stamp ...
  '/image00001.tif'];
% Set the subfolders names - neurofinder specific
opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
