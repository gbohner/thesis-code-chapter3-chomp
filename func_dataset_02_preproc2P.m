function [out_timestamp, saved_opts] = func_dataset_02_preproc2P(preproc2P_stamp, input_opts)
...
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.02.00' ...
...
cell_max_size = 15; %REDUCE THIS - cell_max_size = 19;
opt.spatial_scale = 1.0;%0.5;%1.0;%1.2;
opt.time_scale = 0.2;
...
opt.KS = 8;
opt.mom = 4;
opt.mom_weights = [0.1,1.,1.,1.];%[0.1,1.,1.,1.]; %[0,1,.3,1]; %[0,1,1]; %[0, 0, 0, 1];
opt.init_model = 'neurofinder_preproc2P'; %{'donut_four'}; %{'donut_conv', 'pointlike'};%{'donut_four', 'pointlike'}; %'donut_two'; %'donut_conv'; %'pointlike'; %'filled'; %{'filled', 'donut'};
opt.niter = 1; opt.learn =0; %opt.niter =5;
opt.learn_decomp = 'COV_RAW';
opt.diag_tensors = 1; %1;
opt.diag_cumulants = 1;
opt.standardise_cumulants = 1;
...
opt.mask_image(:,1:25) = 0;
opt.mask_image(:,(end-25):end) = 0;
...
%opt.spatial_push = @(grid_dist)1.0*(grid_dist>=(2*opt.m/3));
...
% Set the new data paths
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.02.00.test' ...
  '/preproc2P/images' preproc2P_stamp ...
  '/image00001.tif'];
% Set the subfolders names - neurofinder specific
opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
...
% Set the new data paths
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.02.01.test' ...
  '/preproc2P/images' preproc2P_stamp ...
  '/image00001.tif'];
% Set the subfolders names - neurofinder specific
opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
