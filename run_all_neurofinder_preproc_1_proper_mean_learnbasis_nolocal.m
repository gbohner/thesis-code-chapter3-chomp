...
input_opts.time_scale = 1.0; % avoid time correlated noise/signal (we only want spatial correlation)
...
input_opts.local_peaks_size = [11,5,0];
input_opts.local_peaks_gauss = 2.;
input_opts.blank_reconstructed = 0;
input_opts.spatial_push = @(x)1.*(x>=10);
input_opts.W_weight_type = 'decomp'; % 'uniform' / 'decomp' - latter using SVD weights to penalise use of bases (but only works with diag_tensors=1 for now)
