...
input_opts.mom = 2;
input_opts.mom_weights = [0.1,1.];
input_opts.NSS = 1;
input_opts.KS = 6;
input_opts.init_model = 'donut_four';%'neurofinder_preproc2P';%; %'donut_four';%{'donut_conv_new', 'filled'};%'neurofinder_preproc2P';%{'donut_conv'}; %{'donut_conv', 'pointlike'};%{'donut_four', 'pointlike'}; %'donut_two'; %'donut_conv'; %'pointlike'; %'filled'; %{'filled', 'donut'};
input_opts.niter = 4; 
input_opts.learn = 1; %input_opts.niter =5;
input_opts.cells_per_image = 200;
input_opts.learn_decomp = 'COV_RAW';%'COV_RAW'; % COV, NMF
input_opts.diag_tensors = 0; %1;
...
savename = ['./Examples/saved_runs_neurofinder_preproc_2_learnbasis' cur_timestamp '.mat'];
