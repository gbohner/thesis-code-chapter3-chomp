function [out_timestamp, saved_opts] = func_dataset_00_preproc2P(preproc2P_stamp, input_opts)
...
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.00.00' ...
...
cell_max_size = 17;
opt.spatial_scale = 1.0;%1.2;%0.5;%1.0;%1.2;
opt.time_scale = 1.0;
opt.denoise_medianfilter = 0;
opt.subtract_background = 1;
opt.log_transform = 0;
opt.whiten = 1;
opt.stabilize = 0;
opt.smooth_filter_sigma = 0.4000;
opt.smooth_filter_mean = 2.4;%cell_max_size*opt.spatial_scale*0.2;%*2;
opt.smooth_filter_var = 4.;%cell_max_size*opt.spatial_scale*0.2;%*2;
...
opt.init_model = 'neurofinder_preproc2P';%; %'donut_four';%{'donut_conv_new', 'filled'};%'neurofinder_preproc2P';%{'donut_conv'}; %{'donut_conv', 'pointlike'};%{'donut_four', 'pointlike'}; %'donut_two'; %'donut_conv'; %'pointlike'; %'filled'; %{'filled', 'donut'};
opt.niter = 1; opt.learn =0; %opt.niter =5;
opt.learn_decomp = 'COV';%'COV_RAW'; % COV, NMF
opt.diag_tensors = 1; %1;
opt.diag_cumulants = 1;
opt.diag_cumulants_offdiagonly = 0;
opt.W_addflat = 0;
opt.W_force_round = 1;
opt.local_peaks = 0;
opt.local_peaks_size = [floor(opt.m+1), floor(opt.m/2), 1];
...
edge_ignore = cell_max_size*1.5;
opt.mask = 1;
opt.mask_image = ones([512,512]);
opt.mask_image(:,1:round(edge_ignore/2.)) = 0;
opt.mask_image(:,(end-round(edge_ignore/2.)):end) = 0;
opt.mask_image(1:round(edge_ignore/2.),:) = 0;
opt.mask_image((end-round(edge_ignore/2.)):end, :) = 0;
...
opt.cells_per_image = 250;
%opt.spatial_push = @(grid_dist)1.0*(grid_dist>=(1*opt.m/4));
...
%opt.spatial_push = @(grid_dist)1.0*(grid_dist>=(1*opt.m/3));
...
%opt.spatial_push = @(grid_dist)(2*(logsig(grid_dist/(opt.m/8))-0.5).*(grid_dist<(2*opt.m))+(grid_dist>=(2*opt.m)));
[opt, ROI_mask, ROIs] = chomp(opt);
...
% Set the new data paths
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.00.00.test' ...
  '/preproc2P/images' preproc2P_stamp ...
  '/image00001.tif'];
% Set the subfolders names - neurofinder specific
opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
...
% Set the new data paths
opt.data_path = [...
  '/nfs/data/gergo/Neurofinder_update/neurofinder.00.01.test' ...
  '/preproc2P/images' preproc2P_stamp ...
  '/image00001.tif'];
% Set the subfolders names - neurofinder specific
opt.input_folder = [fileparts(fileparts(opt.data_path)) '/CHOMP/input/'];
