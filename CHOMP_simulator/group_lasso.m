...
% Slightly modified by Gergo Bohner, 27/07/2016 to include multiple
% measurements (-> timeseries) of b as multiple columns
...
QUIET    = 0;
MAX_ITER = 300;
ABSTOL   = 1e-2; %paper: 1e-3
RELTOL   = 1e-1; %paper: 1e-2
% Data preprocessing
...
function p = objective(A, b, lambda, cum_part, x, z)
    ...
    p = ( 1/2*sum(sum((A*x - b).^2)) + lambda*obj );
end
